...
| P-6 no startup timeout | **Fixed.** `FirebaseAuthRepository.init()` bounds both the persisted-session restore and the first profile read to 4 s (matching the existing `syncEntitlement` timeout pattern), so a stalled network starts the app cold instead of hanging on the splash screen. A code review on PR #9 caught that the first version applied that same 4 s bound inside the shared `_hydrate()` helper, so it also cut off sign-up, sign-in and `completeOnboarding` — a merely-slow (not down) connection there would silently revert a just-completed onboarding write to defaults and bounce the user back into the wizard, since the timeout's catch block falls back to an empty profile. `_hydrate` now takes an optional `timeout` that only `init()` passes; the other callers hydrate without a bound, same as before this fix existed. | — |
