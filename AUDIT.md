...
| R-11 no ProGuard rules, no Crashlytics mapping upload | **Mostly fixed.** Release builds enable `isMinifyEnabled`/`isShrinkResources` with a documented `proguard-rules.pro`; `res/raw/keep.xml` protects the notification icon, which is looked up by string name and invisible to the resource shrinker. CI checks the built APK's native libraries are 16 KB page-aligned (Play's November 2025 requirement) and the manifest explicitly removes the advertising-ID permission — all confirmed working by CI. **Automatic mapping upload is off.** The Crashlytics Gradle plugin (`2.8.1`) applies, but its `uploadCrashlyticsMappingFileRelease` task fails on this project's Gradle 9.1 (`groovy/util/XmlSlurper` — a class the plugin's Groovy usage expects that Gradle 9's newer Groovy doesn't provide at that path; caught by CI, not locally, exactly the risk flagged when this landed). `firebaseCrashlytics { mappingFileUploadEnabled = false }` skips it. Crash *reporting* still works (the Firebase SDK's runtime code, independent of this Gradle plugin); shrunk release stack traces just won't auto-decode to real names in the dashboard until a Gradle-9-compatible plugin version is confirmed and this is turned back on. | — |
