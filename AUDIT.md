...
| P-4 | **P2** | `lib/screens/weight_tracker_screen.dart:255-261`; `lib/state/app_state.dart:99-116`, `:238-252`, `:319-330` | `weights`, `weightHistoryDesc`, and `_sortedByDate` **copy and sort the full list on every getter call**. The history loop calls a getter 3 times per row, so the build is O(n² log n). With daily weigh-ins, a year of data means over 1,000 sorts per build. The rows are built eagerly inside `ListView(children:)`, and the chart plots every point. The new training-log getters have the same shape: `sessions` scans the whole log for each plan slot (`_thisWeekEntryFor`), and `trainingDayKeys` rebuilds a set, on every access. | Sort once per snapshot, or in the query (`orderBy('date', descending: true)`). Cache derived values such as the 7-day average. Use `SliverList.builder` and window the chart (e.g., 90 days). |
...
| D-3 | **P2** | `lib/data/firestore_data_repository.dart:22-30`, `:66-73`, `:83-94` | `watchWeights` and `watchSessions` stream **unbounded** collections. Every cold start or reconnect re-reads every weigh-in ever logged, so reads per launch grow linearly per user. The new `watchTrainingLog` (`:83-94`) has the same shape: every session ever logged is streamed on every launch. | Query only the window the UI shows (`where date >= now-90d`, plus `limit`), and paginate history. Keep an aggregate document (latest weight, 7-day average) updated on write. |
...
| R-11 no ProGuard rules, no Crashlytics mapping upload | **Fixed.** Release builds enable `isMinifyEnabled`/`isShrinkResources` with a documented `proguard-rules.pro`; `res/raw/keep.xml` protects the notification icon, which is looked up by string name and invisible to the resource shrinker. The `com.google.firebase.crashlytics` Gradle plugin (`2.8.1`, matching the FlutterFire-generated template for the installed `firebase_crashlytics` package) uploads the mapping file automatically, so shrunk crash reports still decode to real names. CI also checks the built APK's native libraries are 16 KB page-aligned (Play's November 2025 requirement) and the manifest now explicitly removes the advertising-ID permission. **Not yet device- or Play-console-verified** — this is the first release build with minification on; watch the CI Android job and Crashlytics dashboard once a real crash lands. | — |
